...
then
    ...
    for filename in ./*; do
        ...
        cd ..
        python split_data.py $filename user_id ","
        shortened_name="${filename:0:-4}"
        echo "[INFO] shortened name: $shortened_name"
        if [ "$mode" == "irt" ]
        then
            rnn_prof irt assistments $shortened_name"_big.txt" --onepo --drop-duplicates --no-remove-skill-nans --num-folds 5 --item-id-col problem_id --concept-id-col single &> $shortened_name"-output-estimation"
        else
            if [ "$mode" == "hirt" ]
            then
                rnn_prof irt assistments $shortened_name"_big.txt" --onepo --drop-duplicates --no-remove-skill-nans --num-folds 5 --item-precision 4.0 --template-precision 2.0 --template-id-col template_id --item-id-col problem_id --concept-id-col single &> $shortened_name"-output-estimation"
            else
